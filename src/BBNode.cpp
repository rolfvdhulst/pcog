...
void BBTree::createRootNode() {
   ...
   m_node_data.emplace_back(0,INVALID_BB_NODE,0,0.0,BranchData(INVALID_NODE,INVALID_NODE,BranchType::ROOT));
   m_open_nodes.emplace_back(0);
}
bool BBTree::hasOpenNodes() const {
   return !m_open_nodes.empty();
}
...
void BBTree::createChildren(node_id t_node) {
   BBNode& parent_data = m_node_data[t_node];
   {
      ...
      BBNode left_node(left_id,t_node,parent_data.depth(),parent_data.lowerBound(),
                       BranchData(parent_data.firstBranchingNode(),parent_data.secondBranchNode(),BranchType::DIFFER));
   ...
   }
   {
      ...
      //insert node according to selection rule
      BBNode right_node(right_id,t_node,parent_data.depth(),parent_data.lowerBound(),
                       BranchData(parent_data.firstBranchingNode(),parent_data.secondBranchNode(),BranchType::SAME));
