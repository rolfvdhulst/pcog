...
degree_type DenseSet::size() const noexcept{
   return m_bitset.count();
}

degree_type DenseSet::capacity() const noexcept{
...
bool DenseSet::empty() const noexcept {
   return !m_bitset.any();
}

bool DenseSet::full() const {
   return m_bitset.all();
}

bool DenseSet::any() const {
   return m_bitset.any();
}
...
DenseSet::DenseSet(boost::dynamic_bitset<> t_other)  :m_bitset(std::move(t_other)){

}

DenseSet::Element DenseSet::first() const {
...
DenseSet::ConstIterator DenseSet::end() const{
...
DenseSet &DenseSet::setAll() {
   return setRange(0,capacity()-1);
}
...
DenseSet::ConstIterator::ConstIterator(const DenseSet &t_set, Element t_position) : m_node(t_position),m_denseSet(t_set){
}

DenseSet::Element DenseSet::ConstIterator::operator*() const{
   return m_node;
}

bool DenseSet::ConstIterator::operator==(const DenseSet::ConstIterator &t_other) const {
   assert(this->m_denseSet == t_other.m_denseSet);//we should be iterating over the same set; other things do not make sense
